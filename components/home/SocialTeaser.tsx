...
const SocialTeaser = () => {
  return (
    <section id="socialteaser" className="-mb-0 md:-mb-16">
      <Container className="flex flex-col gap-8 md:gap-16 items-center">
        <Title className="text-xl shrink-0 text-muted-foreground">
          Trusted by top creators
        </Title>
        <div className="grid grid-cols-2 gap-6 md:gap-0 md:flex md:justify-center w-full px-3 md:px-0">
          <CreatorItem
...
const CreatorItem = ({
  ...
  return (
    ...
    >
      ...
      <div className="text-sm lg:text-base mr-auto">
        <h3 className="font-medium whitespace-nowrap">{name}</h3>
        <h4 className="text-muted-foreground text-xs md:text-sm">{username}</h4>
      </div>
      <div
        className="relative w-6 lg:w-9 aspect-square grid place-items-center rounded-md lg:rounded-lg ml-1 md:ml-2 lg:ml-4 shrink-0"
        style={{
          background:
